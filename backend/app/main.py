...
async def start_chat(request: ChatRequest):
    try:
        ...
        # If video not found in status, try to load it as a historical video
        if not video_status:
            logger.info(f"Video {video_id} not found in status, trying to load as historical video")
            try:
                ...
                if os.path.exists(video_dir):
                    ...
                    logger.info(f"Successfully loaded historical video {video_id} in {mode} mode")
                else:
